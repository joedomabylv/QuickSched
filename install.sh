...
sudo sed -v -r -i 's/sample|SAMPLE/quicksched/g' /opt/bitnami/apache2/conf/vhosts/quicksched-vhost.conf;
sudo sed -v -r -i 's/sample|SAMPLE/quicksched/g' /opt/bitnami/apache2/conf/vhosts/quicksched-https-vhost.conf;
...
sudo python /opt/bitnami/projects/quicksched/project/manage.py createsuperuser;

# collect static files for apache to serve
sudo python /opt/bitnami/projects/quicksched/project/manage.py collectstatic --noinput;
